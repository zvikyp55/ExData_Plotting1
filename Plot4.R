...
plot4<-function(){
    ...
    ## Draw the plot
    par(mar=c(4,4,4,2),mfcol=c(2,2)) ##set margins and allow for multiple graphs on one page
    ...
    legend("topright",colnames(data1[7:9]),col=(c("black","red","blue")),lty = 1,bty="n", inset = 0.01)
    axis.POSIXct(1, at=seq(min(data1$DateTime),max(data1$DateTime)+days(1), by="day"), format="%a")  ##label the axis with days
    
    ##plot 3
    plot(data1$DateTime , data1$Voltage,col = "black",xlab="datetime",ylab = "Voltage",type = "l",xaxt="n")
    axis.POSIXct(1, at=seq(min(data1$DateTime),max(data1$DateTime)+days(1), by="day"), format="%a")  ##label the axis with days
    
    ##plot 4
    plot(data1$DateTime , data1$Global_reactive_power,col = "black",xlab="datetime",ylab = "Global_reactive_powerr",type = "l",xaxt="n")
    axis.POSIXct(1, at=seq(min(data1$DateTime),max(data1$DateTime)+days(1), by="day"), format="%a")  ##label the axis with days
